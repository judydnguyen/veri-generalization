...
parser.add_argument('combined_csv', type=Path,
                    default=Path("/home/judy/code/veri-generalization/results/mnist_emnist_pixmix.csv"),
                    nargs='?', help='Path to the combined results CSV')
...
# ── Plot 1: CRA vs epsilon – MNIST ──────────────────────────────────────────
for model_id in model_ids:
    rows = all_results[(all_results['model_id'] == model_id) & (all_results['dataset'] == 'MNIST')]
    if rows.empty:
        continue
...
ax1.set_title('CRA vs Epsilon – MNIST', fontsize=15, fontweight='bold')
ax1.legend(fontsize=9, loc='best')
ax1.set_ylim(bottom=0)

# ── Plot 2: Avg Safe vs Unsafe stacked bar (MNIST / EMNIST / PixMix) ────────
width = 0.25
x = np.arange(len(model_ids))
offsets = {'MNIST': -width, 'EMNIST': 0, 'PixMix': width}
hatch   = {'MNIST': '',     'EMNIST': '---', 'PixMix': '///'}
safe_color   = '#2ecc71'
unsafe_color = '#e74c3c'
...
# ── Plot 3: CRA vs epsilon – MNIST vs EMNIST ─────────────────────────────────
colors_ds    = {'MNIST': '#3498db', 'EMNIST': '#e67e22'}
linestyles   = {'MNIST': '-',       'EMNIST': '--'}
...
ax3.set_title('CRA: MNIST vs EMNIST by Epsilon', fontsize=15, fontweight='bold')
ax3.legend(fontsize=8, loc='best', ncol=2)
ax3.set_ylim(bottom=0)

# ── Plot 4: CRA vs epsilon – MNIST vs PixMix ─────────────────────────────────
colors_ds2  = {'MNIST': '#3498db', 'PixMix': '#e67e22'}
linestyles2 = {'MNIST': '-',       'PixMix': '--'}
