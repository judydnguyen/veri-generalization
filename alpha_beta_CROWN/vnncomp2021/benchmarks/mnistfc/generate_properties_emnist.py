...
def load_data(
    ...
    """
    Loads EMNIST digits data and returns images that are correctly classified
    by all ONNX models. Optionally ensures every class appears at least once.
    
    Note: EMNIST images are transformed to match MNIST orientation (rotate 90° and flip)
    to be compatible with MNIST-trained models.
    """
    ...
    trns_norm = trans.ToTensor()
    # Load EMNIST digits dataset (10 classes: 0-9)
    emnist_test = dset.EMNIST(data_dir, split='digits', train=False, download=True, transform=trns_norm)

    loader_test = DataLoader(emnist_test, batch_size=10000)
    images, labels = next(iter(loader_test))
    
    # Transform EMNIST images to match MNIST orientation
    # EMNIST images need to be rotated 90° clockwise and flipped horizontally
    # to match the orientation expected by MNIST-trained models
    images = torch.rot90(images, k=-1, dims=[2, 3])  # Rotate 90° clockwise
    images = torch.flip(images, dims=[3])  # Flip horizontally
    ...
    # Use reshape instead of view since images may not be contiguous after rotation/flip
    imgs_np = images.reshape(len(images), -1, 1).numpy()  # (N, 784, 1)
...
def save_vnnlib(input_bounds: torch.Tensor, label: int, spec_path: str, total_output_class: int = 10):
    ...
    with open(spec_path, "w") as f:
        f.write(f"; EMNIST digits property with label: {label}\n\n")
...
def generate_vnnlib_dataset(
    ...
    Args:
        ...
        num_images: number of EMNIST digits samples
    ...
    print(f"📦 Generating EMNIST digits VNNLIB dataset in '{save_dir}' for eps={epsilons}")
...
if __name__ == "__main__":
    ...
    # example usage: python generate_properties_emnist.py --epsilons 0.03 --save_dir ./vnnlib_props_0.03 --num_images 20 --random True --seed 42


# generate_vnnlib_dataset([0.03, 0.05], save_dir="./vnnlib_props", num_images=100, seed=42)
# generate_vnnlib_dataset([0.08], save_dir="./vnnlib_props_0.08", num_images=100, seed=42)
# example usage: python generate_properties_emnist.py --epsilons 0.08 --save_dir ./vnnlib_props_0.08 --num_images 20 --random True --seed 42