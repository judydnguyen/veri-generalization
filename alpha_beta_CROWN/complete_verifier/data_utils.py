...
def load_cifar10c():
    """
    Load CIFAR-10-C from an ImageFolder and return:
    - test_data: torchvision ImageFolder with normalization
    - data_max, data_min: normalized clipping bounds (same shape as original code)
    """
    # Where your CIFAR-10C ImageFolder lives
    # e.g. datasets/cifar10-c/...
    database_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'datasets')
    cifar10c_root = os.path.join(database_path, 'cifar10c_by_class')  # adjust folder name if needed
    ...
    # Attach mean/std like in the original
    test_data.mean = torch.tensor(arguments.Config["data"]["mean"])
    test_data.std = torch.tensor(arguments.Config["data"]["std"])

    # Same data_max / data_min logic
    data_max = torch.reshape((1. - test_data.mean) / test_data.std, (1, -1, 1, 1))
    data_min = torch.reshape((0. - test_data.mean) / test_data.std, (1, -1, 1, 1))
...
def load_cifar10c_dataset(spec):
    """Load MNIST/CIFAR*/CIFAR10C test set with normalization."""
    dataset_name = arguments.Config["data"]["dataset"]
    print(f"Trying generic data loader for {dataset_name}.")

    # This now supports MNIST, CIFAR, CIFAR100, CIFAR10C
    test_data, data_max, data_min = load_cifar10c()

    if spec.get('epsilon') is None:
        raise ValueError('You must specify an epsilon')

    # Make epsilon tensor in input space
    eps_temp = make_eps_tensor(spec['epsilon'])

    # Rescale epsilon by std (same logic as before)
    std = torch.tensor(arguments.Config["data"]["std"],
                       dtype=torch.get_default_dtype())
    eps_temp = torch.reshape(eps_temp / std, (1, -1, 1, 1))

    # Use full dataset size as batch_size instead of hardcoding 10_000
    testloader = torch.utils.data.DataLoader(
        test_data,
    ...
    runnerup = None  # keeping interface unchanged
    return X, labels, data_max, data_min, eps_temp, runnerup


# def load_cifar10c_dataset(spec):
#     """Load CIFAR-10C test set with normalization."""
#     print("Trying CIFAR-10C data loader.")
#     test_data, data_max, data_min = load_dataset()
#     if spec['epsilon'] is None:
#         raise ValueError('You must specify an epsilon')
#     eps_temp = make_eps_tensor(spec['epsilon'])
