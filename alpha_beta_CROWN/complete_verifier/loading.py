...
from data_utils import load_cifar10c_dataset, load_eran_dataset, load_sdp_dataset
...
def load_verification_dataset():
    ...
    print(f"Loading verification dataset for {arguments.Config['data']['dataset']}")
    ...
    elif "CIFAR10C" in arguments.Config["data"]["dataset"]:
        dataset_method = load_cifar10c_dataset
        print(f"Using dataset method: {dataset_method}")
...
def parse_run_mode():
    ...
    if (arguments.Config['general']['csv_name']
        ...
        if arguments.Config['specification']['vnnlib_path'] is None:
            ...
            labels = verification_dataset['labels']
            ...
            elif arguments.Config['data']['class_to_verify'] is not None:
                example_idx_list = [i for i in range(X.shape[0]) if labels[i] == arguments.Config['data']['class_to_verify']]
                # print(f'Example indices (total {len(example_idx_list)}): {example_idx_list}')
                print(f"Original input shape: {X.shape}")
                print(f"Labels: {labels}")
                print(colored(f"Total selected examples: {len(example_idx_list)}", 'green'))
                print(colored(f"Class to verify: {arguments.Config['data']['class_to_verify']}", 'green'))
