...
# put this somewhere in your utils module (or file)
import os
import time
import torch
...
def save_adv_example_torchvision(adv_tensor, adv_output, out_dir, prefix="adv", 
                                 value_range=(0, 1), max_images=None):
    """
    Save adversarial example tensor(s) to out_dir using torchvision.save_image.
    If there are multiple images, ALSO save a grid image (square-ish layout).
    """
    # extract dir if out_dir is a file path
    out_dir = os.path.dirname(out_dir)
    os.makedirs(out_dir, exist_ok=True)
    if adv_tensor.dim() == 3:
        adv_tensor = adv_tensor.unsqueeze(0)  # make batch

    # move to CPU and ensure float
    adv_tensor = adv_tensor.detach().cpu().float()

    # clamp to the given range, and bring to [0,1]
    minv, maxv = float(value_range[0]), float(value_range[1])
    adv_tensor = torch.clamp(adv_tensor, minv, maxv)
    norm_tensor = (adv_tensor - minv) / (maxv - minv)
    norm_tensor = torch.clamp(norm_tensor, 0.0, 1.0)

    B = norm_tensor.size(0)
    if max_images is not None:
        B = min(B, int(max_images))
        norm_tensor = norm_tensor[:B]

    timestr = time.strftime("%Y%m%d-%H%M%S")

    # --- Save individual images (preserves previous behavior) ---
    for i in range(B):
        fname = f"{prefix}_{timestr}_{i}.png"
        fpath = os.path.join(out_dir, fname)
        save_image(norm_tensor[i], fpath)

    # --- Additionally save a grid when batch > 1 ---
    if B > 1:
        # square-ish grid
        nrow = int(B ** 0.5)
        if nrow * nrow < B:
            nrow += 1
        grid = make_grid(norm_tensor, nrow=nrow, padding=2)
        grid_name = f"{prefix}_grid_{timestr}.png"
        save_image(grid, os.path.join(out_dir, grid_name))
        print(f"Saved grid image to {os.path.join(out_dir, grid_name)}")

    # optionally save adv_output for analysis (unchanged)
    try:
        import numpy as np
        out_path = os.path.join(out_dir, f"{prefix}_{timestr}_output.npy")
        np.save(out_path, adv_output.detach().cpu().numpy())
    except Exception:
        pass
    ...
    if arguments.Config['general']['save_adv_example']:
        if eval(arguments.Config['attack']['adv_verifier'])(adv_example, adv_output, vnnlib, 
            ...
            try:
                ...
                verified_status = expected_verified_status
                save_adv_example_torchvision(adv_example, adv_output, res_path)
                verified_success = True
