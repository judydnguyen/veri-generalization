...
def create_instances_csv(
    num_props: int = 15,
    path: str = "mnistfc_instances.csv",
    prop_dir: str = "properties",   # subdirectory where .vnnlib files are stored
):
    """
    Creates a CSV file containing only the full paths to the .vnnlib files.

    Each line: /abs/path/to/prop_i_XXX.vnnlib
    """

    cwd = os.getcwd()
    # prop_dir_full = os.path.join(cwd, prop_dir)

    # file names
    prop_names = [f"prop_{i}_0.03.vnnlib" for i in range(num_props)]
    prop_names += [f"prop_{i}_0.05.vnnlib" for i in range(num_props)]

    # full paths
    props = [os.path.join(cwd, p) for p in prop_names]

    with open(path, "w") as f:
        for prop in props:
            f.write(prop + "\n")

    print(f"✅ CSV (paths only) saved to: {path}")
...
# ):
#     """
#     Creates the instances_csv file with full absolute paths.

#     Args:
#         num_props:
#             The number of properties.
#         path:
#             The path of the CSV file to save.
#         prop_dir:
#             Subdirectory containing .vnnlib property files.
#     """

#     # Get absolute directory path based on current working directory
#     cwd = os.getcwd()
#     # prop_dir_full = os.path.join(cwd, prop_dir)
...
#     # Full absolute paths for properties
#     props = [os.path.join(cwd, p) for p in prop_names]

#     # Output path should also be absolute
...
#                 if net == nets[-1] and idx == len(props) - 1:
#                     f.write(line)
#                 else:
#                     f.write(line + "\n")

#     print(f"✅ CSV saved to: {path_full}")

# Example usage:
# create_instances_csv(num_props=15, prop_dir="data/vnnlib")
